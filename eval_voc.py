...
if __name__ == '__main__':
    ...
    for index,image_file in enumerate(file_list):
        ...
        for i in range(num_obj):
            x1 = int(float(image_file[1+5*i]))
            y1 = int(float(image_file[2+5*i]))
            x2 = int(float(image_file[3+5*i]))
            y2 = int(float(image_file[4+5*i]))
            c = int(float(image_file[5+5*i]))
    ...
    for image_path in tqdm(image_list):
        result = predict_gpu(model,image_path,root_path='test/') #result[[left_up,right_bottom,class_name,image_path],]
